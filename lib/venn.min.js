...
(function(venn) {
	...
	venn.VennDiagram = function() {
		var width = 600,
			...
			hashScale = d3.scale.linear().domain([1, 4294967295]).range([ 0, 19.9999 ]);
			...
			selection.each(function(data) {
				...
				enter.append("path")
					...
					.style("fill", function(d) {
						return colours[parseInt(Math.floor(hashScale(hash(colorLabel(d)))))];
					})
					.style("fill-opacity", ".25");

				var enterText = enter.append("text")
					.style("fill", function(d) {
						return d.sets.length == 1 ? colours[parseInt(Math.floor(hashScale(hash(colorLabel(d)))))] : "#444";
					})
		...
		function colorLabel(d) {
			if (d.labels.length == 1) {
				return d.labels[0].qText;
			}
			if (d.label  && d.label.toString().trim().length > 0) return d.label;
			if (d.sets.length == 1) {
				return '' + d.sets[0];
			}
			return d.sets.join('*');
	...
	venn.wrapText = function(circles, labeller) {
		return function() {
			var text = d3.select(this),
				...
				var words = label.split(/(<br>)/).reverse(),
				maxLines = 3,
				...
				lineNumber = 0,
				lineHeight = 1.1, // ems
				tspan = text.html(null).append("tspan").html(word);
			...
			while (true) {
				...
				joined = line.join(" ");
				tspan.html(joined);
				if ((joined.length > minChars && tspan.node().getComputedTextLength() > width) || words.length == 0) {
					line.pop();
					tspan.html(line.join(" "));
					line = [word];
					tspan = text.append("tspan").html(word);
					lineNumber++;
				}
			}
...
(function(venn) {
	...
	venn.greedyLayout = function(areas) {
		...
		for (i = 1; i < mostOverlapped.length; ++i) {
			...
			// YB : 15/03/2018 Allow non overlapped sets
			if (overlap.length === 0) {

				//throw "Need overlap information for set " + JSON.stringify( set );
				positioned[setIndex] = true
				...
				for (var j = 0; j < overlap.length; ++j) {
					...
					var p1 = circles[overlap[j].set],
						d1 = venn.distanceFromIntersectArea(
							set.radius, p1.radius,
							overlap[j].size
					);
					...
					for (var k = j + 1; k < overlap.length; ++k) {
						var p2 = circles[overlap[k].set],
							d2 = venn.distanceFromIntersectArea(
								set.radius, p2.radius,
								overlap[k].size
							)
						;
	...
	/** Given a bunch of sets, and the desired overlaps between these sets
	computes the distance from the actual overlaps to the desired overlaps. 
	...
	venn.intersectionArea = function(circles, stats) {
		...
		if (innerPoints.length > 1) {
			...
			for (i = 0; i < innerPoints.length; ++i) {
				...
				// YB 15/03/2018 Fixed issue "Unable to draw venn diagram"
				if (arc !== null) {
					arcs.push(arc);
